...
namespace Microsoft.ReactNative.Managed.CodeGen.UnitTests.CodeGen
{
  ...
  public class CodeGenMethodTests : CodeGenTestBase
  {
    ...
    {
      TestMethod(methodDecl, isSynchroneous: true);
      TestMethod(methodDecl, isSynchroneous: false);
    }


    private void TestMethod(string methodDecl, bool isSynchroneous)
    {
      TestCodeGen<IMethodSymbol>(
        methodDecl,
        (codeGen, symbol) =>
          codeGen.AddMethod(new ReactMethod(symbol, "MyMethod", isSynchroneous: isSynchroneous)),
        lkgName: isSynchroneous ? "Sync" : "Async");
    }
