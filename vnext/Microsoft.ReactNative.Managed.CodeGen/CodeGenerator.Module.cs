  ...
  public partial class CodeGenerator
  {
    ...
    internal StatementSyntax AddMethod(ReactMethod method)
    {
      ...
      for (int i = 0; i < parameters.Length; i++)
      {
        ...
        if (!method.IsSynchroneous) // IsSynchroneous methods don't have support for the callbacks
        {
          if (i == parameters.Length - 1)
          {
            ...
            if (IsSingleArgCallback(parameters[i].Type, out var argumentType))
            {
              var isReject = parameters.Length >= 2 &&
                             IsSingleArgCallback(parameters[i - 1].Type, out var _);
      ...
      ITypeSymbol? lastParamType = parameters.Length >= 1 ? parameters[parameters.Length - 1].Type : null;
      ITypeSymbol? secondLastParamType = parameters.Length >= 2 ? parameters[parameters.Length - 2].Type : null;
      ...
      else
      {
        ...
        return InvocationStatement(
          MemberAccessExpression(
            ...
            method.IsSynchroneous ? ReactNativeNames.AddSyncMethod : ReactNativeNames.AddMethod),
          LiteralExpression(method.Name),
          MemberAccessExpression(
